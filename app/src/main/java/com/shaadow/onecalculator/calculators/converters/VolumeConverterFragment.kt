...
class VolumeConverterFragment : Fragment() {
    ...
    private val volumeUnits = arrayOf("Liter", "Milliliter", "Cubic Meter", "Cubic Centimeter", "Gallon", "Quart", "Pint", "Cup", "Fluid Ounce")
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        inputVolume = EditText(requireContext()).apply {
            hint = "Enter volume value"
            inputType = android.text.InputType.TYPE_CLASS_NUMBER or android.text.InputType.TYPE_NUMBER_FLAG_DECIMAL
            layoutParams = LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 2f)
        }

        inputLayout.addView(inputLabel)
        inputLayout.addView(inputVolume)
        ...
        spinnerFrom.setSelection(0) // Liter
        spinnerTo.setSelection(1)   // Milliliter
    ...
    private fun convertVolume() {
        val input = inputVolume.text.toString().trim()
        if (input.isEmpty()) {
    ...
    private fun convert(value: Double, from: String, to: String): Double {
        val fromInLiters = value * conversions[from]!!
        return fromInLiters / conversions[to]!!
    }
